...
import {saveGameplayResume,clearGameplayResume} from './resume_state_v078i.js?v=0.7.8i.1';
import {fetchJsonRelease,showDataLoadFailure} from './runtime/data_load_guard_v078i.js?v=0.7.8i.1';
...
const goalBlue=document.getElementById('goalBlue'),goalGreen=document.getElementById('goalGreen'),goalViolet=document.getElementById('goalViolet'),resultTitle=document.getElementById('resultTitle'),starsEl=document.getElementById('stars'),resultSummary=document.getElementById('resultSummary'),bestSummary=document.getElementById('bestSummary'),continueBtn=document.getElementById('continue');
...
function reduced(a,b,c){if(!a&&!b&&!c)return '0:0:0';const g=gcd3(a,b,c);return `${a/g}:${b/g}:${c/g}`}
function countSpecies(){let blue=0,green=0,violet=0;for(const c of occupancy){if(!c)continue;if(c.species==='blue')blue++;else if(c.species==='green')green++;else if(c.species==='violet')violet++}return {blue,green,violet}}
function goalReached(){const {blue,green,violet}=countSpecies(),[gb,gg,gv]=level.goal;return blue>0&&green>0&&violet>0&&blue*gg===green*gb&&blue*gv===violet*gb}
function fusionResultSpecies(a,b){if(a===b)return a;return ['blue','green','violet'].find(s=>s!==a&&s!==b)||null}
...
function updateNetworkAura(){
  ...
  for(let i=0;i<nodeWorld.length;i++){
    const c=nodeCell(i),col=c?.species==='blue'?'rgba(18,145,225,ALPHA)':c?.species==='violet'?'rgba(174,72,255,ALPHA)':'rgba(26,205,130,ALPHA)';
...
function particleColour(species){return species==='blue'?'45,168,255':species==='green'?'43,226,139':species==='violet'?'190,86,255':'70,218,209'}
...
function targetNodes(){
  ...
  if(runtime.targetMode==='fusion')return neighbors(runtime.selectedNode).filter(j=>occupancy[j]?.type==='fusion');
  ...
  if(runtime.targetMode==='imitation'){const src=occupancy[runtime.selectedNode];return neighbors(runtime.selectedNode).filter(j=>occupancy[j]&&occupancy[j].type!=='imitate'&&occupancy[j].species===src?.species)}
  if(runtime.targetMode==='brood')return [...broodRouteMap(runtime.selectedNode).keys()];
  return [];
}
function syncHud(){
  const {blue,green,violet}=countSpecies();movesEl.textContent=`${runtime.moves} ${runtime.moves===1?'MOVE':'MOVES'}`;ratioEl.textContent=`NOW ${reduced(blue,green,violet)}`;
...
function syncResumeState(){saveGameplayResume({colour:'three',campaign:'threenetwork',campaignLabel:'Living Networks',level,index:runtime.levelIndex})}
...
function syncChrome(){
  const world=campaign.worlds[level.world];levelTitleEl.textContent=level.title.en.toUpperCase();levelMetaEl.innerHTML=`<span>3 COLOURS · NETWORKS · W${level.world+1}</span><span>${String(level.local+1).padStart(2,'0')}/${String(world?.count||8).padStart(2,'0')}</span>`;goalBlue.textContent=String(level.goal[0]);goalGreen.textContent=String(level.goal[1]);goalViolet.textContent=String(level.goal[2]);hint.textContent=world?.intro?.en||level.note?.en||'';resultTitle.textContent=level.title.en.toUpperCase();const link=document.getElementById('levelsLink');if(link)link.href=`threecolor_living.html?world=${level.world+1}`;
}
...
class EdgeTransport{
  constructor({sourceNode,targetNode,species,sourceSpecies=species,targetSpecies=species,resultSpecies=species,mode='fusion'}){
    this.sourceNode=sourceNode;this.targetNode=targetNode;this.species=resultSpecies;this.sourceSpecies=sourceSpecies;this.targetSpecies=targetSpecies;this.resultSpecies=resultSpecies;this.mode=mode;this.settings=createSettings(dynamics);Object.assign(this.settings,synapseCanonical.settings);this.settings.speciesA=sourceSpecies;this.settings.speciesB=targetSpecies;sanitizeTransportSettings(this.settings);
    this.action=createTransportAction();this.keys=new AnimationKeyframes();this.keys.load(synapseCanonical.animation||{});
    this.source=new Cell({id:8001,type:'fusion',species:sourceSpecies,position:[...nodeWorld[sourceNode]],rotation:.25,visualSeed:4.1,visualProfile:fusionVisual});
    this.target=new Cell({id:8002,type:'fusion',species:targetSpecies,position:[...nodeWorld[targetNode]],rotation:2.15,visualSeed:7.4,visualProfile:fusionVisual});
    this.moving=new Cell({id:8003,type:'fusion',species:sourceSpecies,position:[...this.source.position],rotation:.25,visualSeed:4.1,visualProfile:fusionVisual});
    this.finalSplit=new Cell({id:8004,type:'split',species:resultSpecies,position:[...this.target.position],rotation:0,visualSeed:8.21,visualProfile:splitVisual});
    ...
    this.cLeft=new Cell({id:8101,type:'fusion',species:sourceSpecies,visualSeed:4.1,visualProfile:fusionVisual});this.cRight=new Cell({id:8102,type:'fusion',species:targetSpecies,visualSeed:7.4,visualProfile:fusionVisual});
  ...
  drawSyn(){const [a,b]=this.endpoints(),pa=cohesivePalette(this.sourceSpecies),pb=cohesivePalette(this.targetSpecies);synapse.draw(baseSynapseOptions(a,b,pa,pb,{...this.params(),curve:this.visualCurve}))}
  drawScaled(scale,opacity=1,pos=this.moving.position){const s=clamp(scale,.16,1.8),v={...fusionVisual,radius:fusionVisual.radius*s,nucleusRadius:fusionVisual.nucleusRadius*s,membraneThickness:(fusionVisual.membraneThickness||.55)*s};this.moving.position=[...pos];renderer.drawCell({time:runtime.simTime,cell:this.moving,visual:v,colors:paletteForSpecies(this.sourceSpecies),opacity,shapeScale:s})}
...
function startFusion(source,target){const a=occupancy[source],b=occupancy[target];if(!a||!b||a.type!=='fusion'||b.type!=='fusion'||!adjacent(source,target))return false;const resultSpecies=fusionResultSpecies(a.species,b.species);if(!resultSpecies)return false;active=new EdgeTransport({sourceNode:source,targetNode:target,species:resultSpecies,sourceSpecies:a.species,targetSpecies:b.species,resultSpecies,mode:'fusion'});active.context='fusion';runtime.mode='acting';runtime.moves++;hint.textContent=a.species===b.species?'Same-colour Fusion.':'Mixed Fusion creates the third colour.';clearSelection();return true}
...
function startSwap(source){hint.textContent='Swap is not available in three-colour mode.';return false}
...
function startImitation(source,target){const a=occupancy[source],b=occupancy[target];if(!a||a.type!=='imitate'||!b||b.type==='imitate'||b.species!==a.species||!adjacent(source,target))return false;localAction={kind:'imitation',source,target,elapsed:0,duration:Math.max(.58,Number(mimicVisual.imitationDuration)||.64),fromSpecies:a.species,toSpecies:a.species,toType:b.type};runtime.mode='acting';runtime.moves++;clearSelection();return true}
...
function handleNode(i){if(runtime.mode!=='playing'||runtime.paused)return;const c=occupancy[i],s=runtime.selectedNode,src=s!=null?occupancy[s]:null;
  if(runtime.targetMode){if(i===s){clearSelection();return}if(runtime.targetMode==='fusion'&&src&&c&&c.type==='fusion'&&src.type==='fusion'&&adjacent(s,i)){startFusion(s,i);return}if(runtime.targetMode==='split'&&src&&!c&&adjacent(s,i)){startSplit(s,i);return}if(runtime.targetMode==='destruct'&&src&&c&&adjacent(s,i)){startDestruct(s,i);return}if(runtime.targetMode==='imitation'&&src&&c&&c.type!=='imitate'&&c.species===src.species&&adjacent(s,i)){startImitation(s,i);return}if(runtime.targetMode==='brood'&&src&&!c&&broodRouteMap(s).has(i)){startBrood(s,i);return}clearSelection();return}
  if(!c)return;
  if(c.type==='fusion'){if(neighbors(i).some(j=>occupancy[j]?.type==='fusion')){runtime.selectedNode=i;runtime.targetMode='fusion';hint.textContent='Choose the Fusion target.'}else hint.textContent='No connected Fusion partner.'}
  ...
  else if(c.type==='imitate'){if(neighbors(i).some(j=>occupancy[j]&&occupancy[j].type!=='imitate'&&occupancy[j].species===c.species)){runtime.selectedNode=i;runtime.targetMode='imitation'}else hint.textContent='Imitation needs a connected cell of the same colour.'}
...
function completeLevel(){if(runtime.mode==='complete')return;runtime.mode='complete';const stars=starsForMoves(runtime.moves);progress=recordCompletion('threenetwork',progress,{levelId:level.id,levelIndex:runtime.levelIndex,moves:runtime.moves,stars,levelCount:campaign.levels.length});clearGameplayResume({colour:'three',campaign:'threenetwork',levelNumber:runtime.levelIndex+1});starsEl.textContent='★'.repeat(stars)+'☆'.repeat(3-stars);starsEl.setAttribute('aria-label',`${stars} of 3 stars`);resultSummary.textContent=`${runtime.moves} ${runtime.moves===1?'MOVE':'MOVES'} · OPTIMAL ${level.min}`;const best=progress.best[level.id];bestSummary.textContent=`BEST ${best.moves} MOVES · ${best.stars}/3 ★`;continueBtn.textContent=runtime.levelIndex<campaign.levels.length-1?'NEXT LEVEL':'CAMPAIGN COMPLETE';window.CellquationSFX?.success();window.CellquationUI?.showResult(resultBackdrop,continueBtn)}
function nextLevel(){if(runtime.levelIndex<campaign.levels.length-1)startLevel(runtime.levelIndex+1);else location.href='threecolor_living.html'}
...
window.addEventListener('error',e=>{errorBox.style.display='block';errorBox.textContent=String(e.error?.stack||e.message||e)});window.addEventListener('unhandledrejection',e=>{errorBox.style.display='block';errorBox.textContent=String(e.reason?.stack||e.reason||e)});init().catch(e=>showDataLoadFailure(e));
