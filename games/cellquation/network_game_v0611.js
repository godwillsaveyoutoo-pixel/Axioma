...
function completeLevel(){if(runtime.mode==='complete')return;runtime.mode='complete';const stars=starsForMoves(runtime.moves);progress=recordCompletion('network',progress,{levelId:level.id,levelIndex:runtime.levelIndex,moves:runtime.moves,stars,levelCount:campaign.levels.length});clearGameplayResume({colour:'two',campaign:'network',levelNumber:runtime.levelIndex+1});starsEl.textContent='★'.repeat(stars)+'☆'.repeat(3-stars);starsEl.setAttribute('aria-label',`${stars} of 3 stars`);resultSummary.textContent=`${runtime.moves} ${runtime.moves===1?'MOVE':'MOVES'} · OPTIMAL ${level.min}`;const best=progress.best[level.id];bestSummary.textContent=`BEST ${best.moves} MOVES · ${best.stars}/3 ★`;continueBtn.textContent=runtime.levelIndex<campaign.levels.length-1?'NEXT LEVEL':'CAMPAIGN COMPLETE';window.CellquationSFX?.success();window.CellquationUI?.showResult(resultBackdrop,continueBtn)}
...
async function init(){
  [synapseCanonical,campaign,layoutPack,visualConfig]=await Promise.all([
    ...
    fetchJsonRelease('./content/runtime/LIVING_NETWORKS_48_V28_RUNTIME.json'),
    fetchJsonRelease('./content/full/LIVING_NETWORKS_LAYOUT_48_V060.json'),
