...
import {CellWorld} from './runtime/world_v0611.js?v=0.7.8i.1';
...
const movesEl=document.getElementById('moves'),ratioEl=document.getElementById('ratioNow'),titleEl=document.getElementById('levelTitle'),metaEl=document.getElementById('levelMeta'),goalBlue=document.getElementById('goalBlue'),goalGreen=document.getElementById('goalGreen');
...
const world=new CellWorld({fusionVisual,splitVisual,broodVisual,destructVisual,switchVisual,mimicVisual,settings:worldSettings});world.setColorMode(2);
...
function counts(){let blue=0,green=0;for(const c of world.cells){if(c.species==='blue')blue++;else if(c.species==='green')green++}return {blue,green}}
function ratio(){const {blue,green}=counts();if(!blue&&!green)return '0:0';const g=gcd(blue,green);return `${blue/g}:${green/g}`}
function goalReached(){const {blue,green}=counts(),[a,b]=level.goal;return blue>0&&green>0&&blue*b===green*a}
...
function syncChrome(){
  titleEl.textContent=level.title.en.toUpperCase();metaEl.innerHTML=`<span>FOUNDATIONS · W${level.world+1}</span><span>${String(level.local+1).padStart(2,'0')}/${String(campaign.worlds[level.world]?.count||6).padStart(2,'0')}</span>`;
  goalBlue.textContent=level.goal[0];goalGreen.textContent=level.goal[1];hint.textContent=campaign.worlds[level.world]?.intro?.en||level.note?.en||'';resultTitle.textContent=level.title.en.toUpperCase();
  document.getElementById('levelsLink').href=`foundations.html?world=${level.world+1}`;
}
...
function addSpec(spec,pos,i){
  ...
  if(spec.role==='swap')return world.addSwap(pos,[0,0],opts);
...
function syncResumeState(){saveGameplayResume({colour:'two',campaign:'foundations',campaignLabel:'Foundations',level,index:runtime.index})}
...
function completeLevel(){if(runtime.mode==='complete')return;runtime.mode='complete';const stars=starsForMoves(runtime.moves);progress=recordCompletion('foundations',progress,{levelId:level.id,levelIndex:runtime.index,moves:runtime.moves,stars,levelCount:campaign.levels.length});clearGameplayResume({colour:'two',campaign:'foundations',levelNumber:runtime.index+1});starsEl.textContent='★'.repeat(stars)+'☆'.repeat(3-stars);starsEl.setAttribute('aria-label',`${stars} of 3 stars`);resultSummary.textContent=`${runtime.moves} ${runtime.moves===1?'MOVE':'MOVES'} · OPTIMAL ${level.min}`;const best=progress.best[level.id];bestSummary.textContent=`BEST ${best.moves} MOVES · ${best.stars}/3 ★`;continueBtn.textContent=runtime.index<campaign.levels.length-1?'NEXT LEVEL':'CAMPAIGN COMPLETE';window.CellquationSFX?.success();window.CellquationUI?.showResult(resultBackdrop,continueBtn)}
...
function nextLevel(){if(runtime.index<campaign.levels.length-1)startLevel(runtime.index+1);else location.href='foundations.html'}
...
function pointerAction(ev){ev.preventDefault();if(runtime.mode!=='playing'||runtime.paused)return;const [x,y]=renderer.screenToWorld(ev.clientX,ev.clientY),cell=world.findCellAt(x,y);
  if(pendingImitate){const src=pendingImitate;if(!cell||cell===src||cell.type==='imitate'){cancelPending();return}src.setSelected(false);pendingImitate=null;if(world.beginImitation(src,cell)){runtime.moves++;hint.textContent=`Imitation copies ${cell.species} ${cell.type}.`}syncHud();return}
  ...
  if(cell.type==='swap'){if(world.isBusy)return;if(world.beginSwap(cell)){runtime.moves++;hint.textContent='Colour switched.'}syncHud();return}
  ...
  if(cell.type==='imitate'){if(world.isBusy)return;world.clearSelection();pendingImitate=cell;cell.setSelected(true);syncHud();return}
