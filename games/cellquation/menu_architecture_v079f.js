/* Cellquation v0.7.9f — production navigation runtime. */
...
const CAMPAIGNS={
  foundations:{colour:'two',label:'FOUNDATIONS',modeLabel:'2 COLOUR',path:'./content/runtime/FOUNDATIONS_30_RUNTIME.json',browser:'foundations.html',play:'play.html',tutorial:'tutorial.html?mode=2f',accent:'foundation',icon:'foundation',worldCount:5,levelCount:30},
  network:{colour:'two',label:'LIVING NETWORKS',modeLabel:'2 COLOUR',path:'./content/runtime/LIVING_NETWORKS_48_V28_RUNTIME.json',browser:'living.html',play:'living_play.html',accent:'network',icon:'network',worldCount:6,levelCount:48},
  threefoundations:{colour:'three',label:'FOUNDATIONS',modeLabel:'3 COLOUR',path:'./content/threecolor/FOUNDATIONS_FULL_30_V071.json',browser:'threecolor_foundations.html',play:'threecolor_play.html',tutorial:'tutorial.html?mode=3f',accent:'three',icon:'foundation',worldCount:5,levelCount:30},
  threenetwork:{colour:'three',label:'LIVING NETWORKS',modeLabel:'3 COLOUR',path:'./content/threecolor/LIVING_NETWORKS_FULL_48_V071.json',browser:'threecolor_living.html',play:'threecolor_living_play.html',accent:'network',icon:'network',worldCount:6,levelCount:48},
};
...
async function getCampaign(key){
  ...
  const c=await fetchJsonRelease(cfg.path);
...
function cardArt(kind,colour){
  const mode=colour==='three'?'3c':'2c';
  const file=kind==='network'?`menu_living_${mode}.webp`:`menu_foundations_${mode}.webp`;
  return `<img class="cq-campaign-art" src="assets/menu/${file}?v=0.7.9f" alt="" aria-hidden="true" decoding="async" loading="eager">`;
}

function card({url,accent='foundation',icon='foundation',colour='two',kicker,title,desc,meta=[],progress=0,pending=false}){
  const a=href(el('a',`cq-card${pending?' is-hydrating':''}`),url);if(pending)a.setAttribute('aria-busy','true');a.dataset.accent=accent;const ic=el('div','cq-card__icon');ic.innerHTML=cardArt(icon,colour);a.append(ic);
  ...
  const m=el('div','cq-card__meta');meta.forEach(x=>m.append(el('span','',x)));if(pending&&!meta.length)m.append(el('span','cq-card__meta-reserved',''));const bar=progressBar(progress);if(pending)bar.setAttribute('aria-hidden','true');copy.append(m,bar);a.append(copy,el('div','cq-card__chev','›'));return a;
}
...
function clearResume(colour){const all=loadResume();if(!(colour in all))return;delete all[colour];try{if(Object.keys(all).length)localStorage.setItem(RESUME_KEY,JSON.stringify(all));else localStorage.removeItem(RESUME_KEY)}catch(_){ }}
function validatedResume(colour,key,campaign,progress){
  const resume=loadResume()[colour],cfg=resume&&CAMPAIGNS[resume.campaign];
  if(!resume||resume.campaign!==key||!cfg||cfg.colour!==colour||resume.source!=='gameplay'){if(resume&&resume.campaign===key)clearResume(colour);return null}
  const levelNumber=Number(resume.level);
  if(!Number.isInteger(levelNumber)||levelNumber<1||levelNumber>campaign.levels.length){clearResume(colour);return null}
  const level=campaign.levels[levelNumber-1];
  if(!level||String(resume.levelId||'')!==String(level.id||'')||progress?.best?.[level.id]){clearResume(colour);return null}
  const normalized={...resume,campaign:key,campaignLabel:cfg.label==='FOUNDATIONS'?'Foundations':'Living Networks',level:levelNumber,world:(Number(level.world)||0)+1,local:(Number(level.local)||0)+1,title:titleText(level.title),levelId:String(level.id||''),source:'gameplay'};
  if(JSON.stringify(normalized)!==JSON.stringify(resume))saveResume(colour,normalized);
  return normalized;
}
...
async function home(){
  ...
  function drawContinue(){
    ...
    const raw=loadResume()[colour],resumeData=raw&&data[raw.campaign];
    const resume=raw&&resumeData?validatedResume(colour,raw.campaign,resumeData.campaign,resumeData.progress):null;
    const cfg=resume&&CAMPAIGNS[resume.campaign];
    if(!resume||!cfg){continueAction.hidden=true;continueAction.removeAttribute('href');continueAction.replaceChildren();return}
  ...
  function draw(){
    ...
    const [fk,nk]=campaignKeys(colour),f=data[fk],n=data[nk],three=colour==='three',fc=CAMPAIGNS[fk],nc=CAMPAIGNS[nk];
    const fPending=!f,nPending=!n;
    if(modeMeta)modeMeta.textContent=three?'3 COLOUR · EXPERT SYSTEM':'2 COLOUR · CLASSIC SYSTEM';
    root.toggleAttribute('aria-busy',fPending||nPending);
    root.replaceChildren(
      card({url:browserUrl(fk,f?.campaign.worlds.length||fc.worldCount),accent:three?'three':'foundation',icon:'foundation',colour:three?'three':'two',kicker:`${f?.campaign.worlds.length||fc.worldCount} WORLDS · ${f?.total||fc.levelCount} LEVELS`,title:'Foundations',desc:three?'Three-colour puzzles in free space.':'Learn the cell actions in free space.',meta:f?[`${f.done}/${f.total} complete · ${f.stars} ★`]:[],progress:f?pct(f.done,f.total):0,pending:fPending}),
      card({url:browserUrl(nk,n?.campaign.worlds.length||nc.worldCount),accent:'network',icon:'network',colour:three?'three':'two',kicker:`${n?.campaign.worlds.length||nc.worldCount} WORLDS · ${n?.total||nc.levelCount} LEVELS`,title:'Living Networks',desc:three?'Three colours inside connected systems.':'Connected puzzles where routes matter.',meta:n?[`${n.done}/${n.total} complete · ${n.stars} ★`]:[],progress:n?pct(n.done,n.total):0,pending:nPending})
    );
  ...
  draw();await ensureColour(colour);draw();
  toggle.addEventListener('click',async e=>{
    const b=e.target.closest('button[data-colour]');if(!b||b.dataset.colour===colour)return;
    const next=b.dataset.colour;colour=next;main.dataset.colour=colour;
...
async function browser(){
  ...
  try{
    ...
    if(ref.origin===location.origin&&ref.pathname.endsWith('/'+cfg.play))refFromPlay=ref;
  }catch(_){ }
  ...
  function drawWorld(push=false,direction=0){
    const w=c.worlds[wi],ls=c.levels.slice(w.offset,w.offset+w.count),resume=validatedResume(cfg.colour,key,c,p);
...
try{if(screen==='home')await home();else if(screen==='browser')await browser()}catch(err){showDataLoadFailure(err,{title:'The menu data could not be loaded',detail:'Cellquation could not read the campaign data. Check your connection, then reload the data.'})}
