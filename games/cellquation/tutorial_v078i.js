...
const STEPS_2=[
  {title:'Ratios can scale',text:'The goal is a proportion, not an exact total. A target of 1 : 2 is also matched by 2 : 4 or 3 : 6.',label:'1 : 2 = 2 : 4 = 3 : 6',kind:'ratio',specs:[['fusion','blue',-.31,.18],['fusion','blue',-.31,-.18],['fusion','green',.07,.34],['fusion','green',.34,.12],['fusion','green',.07,-.12],['fusion','green',.34,-.34]]},
...
const STEPS_3=[
  {title:'Ratios can scale',text:'The goal is a proportion, not an exact total. A target of 1 : 2 : 1 is also matched by 2 : 4 : 2.',label:'1 : 2 : 1 = 2 : 4 : 2',kind:'ratio',specs:[['fusion','blue',-.34,.20],['fusion','blue',-.34,-.20],['fusion','green',0,.38],['fusion','green',.22,.14],['fusion','green',0,-.14],['fusion','green',.22,-.38],['fusion','violet',.50,.20],['fusion','violet',.50,-.20]]},
...
function renderStep(){
  ...
  prev.disabled=stepIndex===0;prev.style.opacity=stepIndex===0?'.38':'1';demo.classList.remove('done');resetWorld();
  if(s.kind==='ratio'){completed=true;next.disabled=false;next.textContent='GOT IT';tryHint.textContent='EQUIVALENT PROPORTIONS COUNT';return}
  next.disabled=true;next.textContent='TRY IT';tryHint.textContent='TRY THE ACTION ABOVE';
}
...
function resultCorrect(){
  const k=steps[stepIndex].kind;
  if(k==='fusion'){const c=world.cells.find(c=>c.type==='split');if(!c)return false;if(three&&stepIndex===1)return c.species==='violet';return true}
...
window.__CELLQUATION_TUTORIAL__={build:'v0.7.8i.1-ratio-clarity-tutorial',mode,get step(){return stepIndex},get world(){return world},get completed(){return completed}};
