...
export class CellWorld {
  ...
  #startFusionTransition(transitionController, now) {
    ...
    // v0.7.8i.1: prime frame 0 immediately. #makeFusionTransition removes the
    // two source cells from the ordinary render list, so leaving
    // currentMechanics=null until the next simulation tick creates a single
    // blank frame exactly at the approach -> fusion handoff.
  ...
  beginDivision(splitCell, transitionController, now) {
    ...
    if (!this.activeTransition && !this.activeApproach) {
      ...
      // v0.7.8i.1: same frame-0 handoff protection as Fusion. The Split cell
      // has already left this.cells, so the transition must be drawable now,
      // not one simulation tick later.
      const mechanics=evaluateFusionSplitMechanics({
