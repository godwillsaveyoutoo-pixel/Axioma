/* Cellquation v0.7.9f.2.1 — unified Settings + orientation-aware persistent playfield backgrounds. */
(function(){
  'use strict';
  const BG_KEY='cellquation.settings.background.v1';
  const BACKGROUNDS=[
    {id:'abyss',title:'Abyss Void',desc:'Deep, sparse and calm.',srcPortrait:'assets/backgrounds/options/abyss_void_portrait.webp',srcLandscape:'assets/backgrounds/options/abyss_void_landscape.webp',thumb:'assets/backgrounds/thumbs/abyss_void.jpg',posPortrait:'50% 50%',posLandscape:'50% 50%'},
    {id:'reef',title:'Bioluminescent Reef',desc:'Lively glowing life and stronger contrast.',srcPortrait:'assets/backgrounds/options/bioluminescent_reef_portrait.webp',srcLandscape:'assets/backgrounds/options/bioluminescent_reef_landscape.webp',thumb:'assets/backgrounds/thumbs/bioluminescent_reef.jpg',posPortrait:'50% 50%',posLandscape:'50% 50%'},
    {id:'trench',title:'Midnight Trench',desc:'Very dark, quiet and focused.',srcPortrait:'assets/backgrounds/options/midnight_trench_portrait.webp',srcLandscape:'assets/backgrounds/options/midnight_trench_landscape.webp',thumb:'assets/backgrounds/thumbs/midnight_trench.jpg',posPortrait:'50% 52%',posLandscape:'50% 50%'},
    {id:'emerald',title:'Emerald Depths',desc:'Teal-green flora with organic depth.',srcPortrait:'assets/backgrounds/options/emerald_depths_portrait.webp',srcLandscape:'assets/backgrounds/options/emerald_depths_landscape.webp',thumb:'assets/backgrounds/thumbs/emerald_depths.jpg',posPortrait:'50% 50%',posLandscape:'50% 50%'},
    {id:'quiet',title:'Quiet Ocean',desc:'Soft open water with minimal visual noise.',srcPortrait:'assets/backgrounds/options/quiet_ocean_portrait.webp',srcLandscape:'assets/backgrounds/options/quiet_ocean_landscape.webp',thumb:'assets/backgrounds/thumbs/quiet_ocean.jpg',posPortrait:'50% 48%',posLandscape:'50% 50%'}
  ];
  const isLandscape=()=>window.innerWidth>window.innerHeight;
  const orientationName=()=>isLandscape()?'landscape':'portrait';
  function resolvedBackground(bg){
    const landscape=isLandscape();
    return {...bg,orientation:landscape?'landscape':'portrait',src:landscape?bg.srcLandscape:bg.srcPortrait,pos:landscape?(bg.posLandscape||'50% 50%'):(bg.posPortrait||'50% 50%')};
  }
  ...
  function backgroundDetail(i=currentIndex){
    const index=normalizeIndex(i),bg=resolvedBackground(BACKGROUNDS[index]);
  ...
  function applyNow(i,{persist=true}={}){
    currentIndex=normalizeIndex(i);const bg=resolvedBackground(BACKGROUNDS[currentIndex]),root=document.documentElement;
    root.style.setProperty('--cq-playfield-bg',cssUrl(bg.src));
    root.style.setProperty('--cq-playfield-pos',bg.pos||'50% 50%');
    root.dataset.cqBackground=bg.id;root.dataset.cqBackgroundOrientation=bg.orientation;
    if(persist)saveIndex(currentIndex);announceBackground(currentIndex);syncBackgroundUI();
  }
  function selectBackground(i,{preload=true,scroll=true,persist=true}={}){
    i=normalizeIndex(i);const bg=resolvedBackground(BACKGROUNDS[i]);const token=++backgroundRequest;
    if(scroll)scrollBackgroundTo(i,true);
    if(!preload){applyNow(i,{persist});return Promise.resolve(bg)}
    document.documentElement.classList.add('cq-background-loading');
    return new Promise(resolve=>{
      const image=new Image();
      image.onload=()=>{if(token===backgroundRequest)applyNow(i,{persist});document.documentElement.classList.remove('cq-background-loading');resolve(bg)};
      image.onerror=()=>{document.documentElement.classList.remove('cq-background-loading');resolve(bg)};
      image.src=bg.src;
    });
  }
  let lastOrientation=orientationName(),orientationTimer=0;
  function syncOrientation(){
    const next=orientationName();if(next===lastOrientation)return;lastOrientation=next;
    clearTimeout(orientationTimer);orientationTimer=setTimeout(()=>selectBackground(currentIndex,{preload:true,scroll:false,persist:false}),90);
  }
  window.addEventListener('resize',syncOrientation,{passive:true});window.addEventListener('orientationchange',syncOrientation,{passive:true});
  // Apply the persisted choice immediately so there is no default-background flash.
  {const bg=resolvedBackground(BACKGROUNDS[currentIndex]),root=document.documentElement;root.style.setProperty('--cq-playfield-bg',cssUrl(bg.src));root.style.setProperty('--cq-playfield-pos',bg.pos);root.dataset.cqBackground=bg.id;root.dataset.cqBackgroundOrientation=bg.orientation;window.CellquationBackgroundState=backgroundDetail(currentIndex);}
