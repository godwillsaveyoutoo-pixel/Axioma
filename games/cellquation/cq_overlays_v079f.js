/* Cellquation v0.7.9f — overlay interaction hardening. */
(function(){
  'use strict';
  const doc=document;
  ...
  function enhanceSettings(){
    doc.querySelectorAll('.pause-card[data-cq-unified-settings="1"]').forEach(card=>{
      card.dataset.cqOverlaySystem='v079f';ensureClose(card);
      const tabs=[...card.querySelectorAll('[data-cq-settings-tab]')];
      tabs.forEach((tab,index)=>{
        if(tab.dataset.cq79fKeys==='1')return;tab.dataset.cq79fKeys='1';
  ...
  function syncOpenFocus(){
    for(const overlay of [doc.getElementById('settingsOverlay'),doc.getElementById('pauseOverlay')]){
      if(!overlay)continue;
      if(visible(overlay)){
        if(overlay.dataset.cq79fOpenFocus==='1')continue;
        overlay.dataset.cq79fOpenFocus='1';
        requestAnimationFrame(()=>overlay.querySelector('[data-cq-settings-tab].is-active,[data-cq-settings-tab],button,a[href]')?.focus());
      }else delete overlay.dataset.cq79fOpenFocus;
    }
    const result=doc.getElementById('resultBackdrop');
    if(result){
      const open=visible(result)&&result.classList.contains('show');
      if(open&&result.dataset.cq79fOpenFocus!=='1'){
        result.dataset.cq79fOpenFocus='1';
        requestAnimationFrame(()=>result.querySelector('.result-actions .primary,.result-actions button,button')?.focus());
      }else if(!open)delete result.dataset.cq79fOpenFocus;
    }
    const failure=doc.getElementById('cqDataLoadFailure');
    if(failure&&failure.dataset.cq79fOpenFocus!=='1'){
      failure.dataset.cq79fOpenFocus='1';
      requestAnimationFrame(()=>failure.querySelector('button,[href]')?.focus());
    }
  }
  ...
  function observe(){
    ...
    const result=doc.getElementById('resultBackdrop');
    if(result){
      const resultObserver=new MutationObserver(syncOpenFocus);
      resultObserver.observe(result,{attributes:true,attributeFilter:['class','aria-hidden']});
    }
  }
  function init(){
    doc.documentElement.dataset.cqOverlaySystem='v079f';
    doc.body?.setAttribute('data-overlay-ui','v079f');
